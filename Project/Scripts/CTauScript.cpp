...
CTauScript::CTauScript()
	: CMonsterScript(SCRIPT_TYPE::TAUSCRIPT)
	, m_CurrentState(eTauState::Idle)
	, m_PrevState(eTauState::Idle)
	, m_StateTimer(0.f)
	, m_ChaseCoolTime(0.f)
...
void CTauScript::Begin()
{
	...
	for (size_t i = 0; i < child.size(); i++)
	{
		...
		wstring name = L"Tau Attack Area " + to_wstring(i);
...
void CTauScript::Tick()
{
	...
	case eTauState::Move:
		Move();
		break;
	case eTauState::Patrol:
		Patrol();
		break;
	case eTauState::Attack:
		Attack();
		break;
	case eTauState::Rush:
		Rush();
...
void CTauScript::BeginOverlap(CCollider2D* ownerCollider, CGameObject* otherObject, CCollider2D* otehrCollider)
{
...
void CTauScript::Overlap(CCollider2D* ownerCollider, CGameObject* otherObject, CCollider2D* otehrCollider)
{
	if (otherObject->GetName() == L"SaintWall_0_eff")
		KnockBack(otherObject, L"SaintWall");
}

void CTauScript::EndOverlap(CCollider2D* ownerCollider, CGameObject* otherObject, CCollider2D* otehrCollider)
{
	if (otherObject->GetName() == L"SaintWall_0_eff")
...
void CTauScript::SetState(eTauState nextState)
{
	if (m_CurrentState != nextState)
	{
		...
		{
		case eTauState::Idle:
			PlayAnimation(L"Tau_Idle", true);
			break;
		case eTauState::Move:
			PlayAnimation(L"Tau_Move", true);
			break;
		case eTauState::Patrol:
			PlayAnimation(L"Tau_Move", true);
			break;
		case eTauState::Rush:
			PlayAnimation(L"Tau_Attack_1", false);
			break;
		case eTauState::Buff:
			PlayAnimation(L"Tau_Skill", false);
			break;
		case eTauState::Death:
			PlayAnimation(L"Tau_Death", false);
			break;
		}
	}
}

void CTauScript::Idle()
{
...
void CTauScript::Move()
{
	if (m_Player)
	{
		...
		if (IsPlayerTarget())
		{
			SetState(eTauState::Attack);
		}
		else if (Vec3::Distance(playerPos, tauPos) > m_TraceDistance)
		{
			SetState(eTauState::Patrol);
		}
	}
}

void CTauScript::Patrol()
{
	CPlayerScript* pPlayerScript = m_Player->GetScript<CPlayerScript>();
...
void CTauScript::Attack()
{
	...
	if (m_CurAttackCoolTime <= 0.f)
	{
		...
		{
			PlayAnimation(L"Tau_Attack_0", false);
...
void CTauScript::UpdataDirection()
{
	if (m_Player)
	{
		CPlayerScript* pPlayerScript = m_Player->GetScript<CPlayerScript>();
...
bool CTauScript::IsPlayerTarget()
{
	if (!m_Player)
		return false;
...
void CTauScript::AnimationInit()
{
	GetOwner()->Animator2D()->LoadAnimation(L"Animation\\Tau_Idle.anim");
	GetOwner()->Animator2D()->LoadAnimation(L"Animation\\Tau_Move.anim");
	GetOwner()->Animator2D()->LoadAnimation(L"Animation\\Tau_Attack_0.anim");
	GetOwner()->Animator2D()->LoadAnimation(L"Animation\\Tau_Attack_1.anim");
	GetOwner()->Animator2D()->LoadAnimation(L"Animation\\Tau_Skill.anim");
	GetOwner()->Animator2D()->LoadAnimation(L"Animation\\Tau_Death.anim");
}